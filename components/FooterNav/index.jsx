...
const FooterNav = () => {
  ...
  return (
    ...
    <div className="w-full text-zinc-200 ">
        ...
        {navItems.map((item, i) => (
            <Link href={item.href} key={i} className={cn('flex items-center flex-col gap-0.5 size-full relative transition py-[10px]', pathname === item.href ? 'text-green-500' : 'justify-center')}>
                <div className={cn(pathname !== item.href ? "flex items-center justify-center":null)}>
                    {pathname === item.href ? <item.mIcon.solid className="size-5 transition" /> : <item.mIcon.outlined className="size-6 transition" />}
                </div>
